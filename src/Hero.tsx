...
export default function Hero() {
  return (
    <section className="hero-section-scroll relative flex w-full min-w-0 shrink-0 flex-col overflow-hidden px-4 pt-8 pb-[max(1rem,env(safe-area-inset-bottom,0px))] mobile:pt-3 max-[1274px]:min-h-0 max-[1274px]:flex-1 max-[1274px]:justify-start sm:px-6 sm:pt-10 sm:pb-[max(1.25rem,env(safe-area-inset-bottom,0px))] min-[1275px]:min-h-[calc(100svh-7rem)] min-[1275px]:px-8 min-[1275px]:py-4 min-[1275px]:pt-10 min-[1275px]:pb-4">
      <div className="relative z-10 mx-auto flex min-h-0 min-w-0 w-full max-w-[1685px] flex-1 flex-col gap-6 pb-4 mobile:gap-4 sm:gap-8 min-[1275px]:gap-8">
        {/*
          ≤1274px: copy + CTAs → spacer → contract.
          ≥1275px: copy, then one row — CTAs left, contract bar right (full width).
        */}
        <div className="order-1 flex min-h-0 w-full max-w-full min-w-0 shrink-0 flex-col items-center gap-3 mobile:gap-2 sm:gap-4 pt-5 mobile:pt-1 sm:pt-7 max-[1274px]:items-stretch min-[1275px]:mt-3 min-[1275px]:max-w-[848.7px] min-[1275px]:w-full min-[1275px]:shrink-0 min-[1275px]:items-start min-[1275px]:gap-4 min-[1275px]:pt-0">
          <HeroLaunchLine />
