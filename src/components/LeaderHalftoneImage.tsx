...
/** Halftone: #2073A8 field, #014778 dots; per-image luma stretch preserves face detail on dark portraits. */
const BG = '#2073A8'
const INK = '#014778'
...
function softenSideBackground(
  ...
  let soft = 0
  if (nx > SIDE_RIM_START) {
    const ts = (nx - SIDE_RIM_START) / (1 - SIDE_RIM_START)
...
export default function LeaderHalftoneImage({ src, alt, className, loading = 'lazy' }: LeaderHalftoneImageProps) {
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="group"
      aria-label={`${alt}. Halftone; hover or focus for photo.`}
      className={[
